...
public: 
	struct PlayerHand {
		...
		PlayerHand(const CardSet<HoldEmRank, Suit>& handSet, const std::string& playerName, HoldEmHandRank handRank) : hand(handSet), name(playerName), rank(handRank) {}
	};
...
protected:
    ...
    CardSet<HoldEmRank, Suit> shared;  // The shared board of cards

    // Protected method to deal cards
    virtual void deal();

    HoldEmHandRank holdem_hand_evaluation(const CardSet<HoldEmRank, Suit>& playerHand);
    HoldEmHandRank evaluate_hand(const std::vector<Card<HoldEmRank, Suit>>& cards_value);
public:
