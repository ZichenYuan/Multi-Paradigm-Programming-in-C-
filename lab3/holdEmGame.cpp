...
HoldEmHandRank HoldEmGame::holdem_hand_evaluation(const CardSet<HoldEmRank, Suit>& playerHand) {
    ...
    handCopy.sort();
    std::vector< Card<HoldEmRank, Suit> > cards(handCopy.begin(), handCopy.end());
...
bool operator<(const HoldEmGame::PlayerHand& lhs, const HoldEmGame::PlayerHand& rhs) {
    ...
    //     return a.getRank() > b.getRank();
    // });
    CardSet<HoldEmRank, Suit> lhsCards(lhs.hand);
    CardSet<HoldEmRank, Suit> rhsCards(rhs.hand);

    // sort cards in descending order to compare (lab3)
    lhsCards.sort();
    std::reverse(lhsCards.begin(),lhsCards.end());
    rhsCards.sort();
    std::reverse(rhsCards.begin(),rhsCards.end());
    std::vector<Card<HoldEmRank, Suit>> sortedLhs (lhsCards.begin(), lhsCards.end());
    std::vector<Card<HoldEmRank, Suit>> sortedRhs (rhsCards.begin(), rhsCards.end());
...
int HoldEmGame::play(){
    while(true){
     ...
     for (auto& playerEvaluation : playerEvaluations) {
            // CardSet<HoldEmRank, Suit> handCopy(playerEvaluation.hand);
            // // handCopy.sort();
            // std::vector< Card<HoldEmRank, Suit> > hand(playerEvaluation.hand.begin(), playerEvaluation.hand.end());
            ...
            // handCopy.insert(handCopy.end(), shared.begin(), shared.end());
            playerEvaluation.hand.add_cards(shared);
