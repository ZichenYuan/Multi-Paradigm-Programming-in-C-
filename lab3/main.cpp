...
int main(int argc, const char *argv[]) {
    ...
    if (argc < 2) {
        ...
        std::cout << "GoFish needs 2-5 players." << std::endl;
    ...
    // Check for "HoldEm" game type and ensure there are 2 ~ 9 players
        ...
        if (argc < 5 || argc > 8) {
            std::cout << "Usage: " << argv[0] << " GoFish <Player1> <Player2> [<Player3> ... <Player5>]" << std::endl;
            std::cout << "GoFish needs 2-5 players." << std::endl;
            return INVALID_PLAYER_NUMBER;
        }
    }
...
std::shared_ptr<Game> create(int argc, const char *argv[]) {
    ...
    if (gameName == "Pinochle") {
        ...
        try {
            ...
            {
                game = std::make_shared<GoFishGame<Suit, HoldEmRank, HoldEmDeck> >(argc, argv);
            }
            else  if  (strcmp(argv[2], "Pinochle") == 0)
            {
                game = std::make_shared<GoFishGame<Suit, PinochleRank, PinochleDeck> >(argc, argv);
            }
            else  if  (strcmp(argv[2], "Uno") == 0)
            {
                game = std::make_shared<GoFishGame<UnoColor, UnoRank, UnoDeck> >(argc, argv);
            }
            else {
                throw std::invalid_argument("Valid Decks for GoFish are HoldEm, Pinochle or Uno");
            }
        }
